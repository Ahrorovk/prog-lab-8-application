...
class RemoveByIdUseCase @Inject constructor(
    private val repository: LabWorkRepository
) {
    operator fun invoke(token: String, request: RemoveByIdRequest): Flow<Resource<Response>> = flow {
        try {
            emit(Resource.Loading<Response>())
            val response = repository.removeById(token, request)
