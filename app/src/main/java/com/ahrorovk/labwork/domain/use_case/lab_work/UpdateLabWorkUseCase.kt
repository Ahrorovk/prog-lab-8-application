...
class UpdateLabWorkUseCase @Inject constructor(
    private val repository: LabWorkRepository
) {
    operator fun invoke(token: String, updateLabWorkRequest: UpdateLabWorkRequest): Flow<Resource<Response>> = flow {
        try {
            emit(Resource.Loading<Response>())
            val response = repository.update(token, updateLabWorkRequest)
